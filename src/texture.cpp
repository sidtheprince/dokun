...
/////////////
Texture::Texture(void * data, int width, int height, int depth, int channel) : rect(0, 0, 0, 0), type("diffuse")
{
...
/////////////
bool Texture::load(void * data, int width, int height, int depth, int channel)
{
    this->data    = data;
	this->width   = width;
	this->height  = height;
...
int Texture::get_buffer(lua_State *L)
{
	...
	{
		Texture * texture = *static_cast<Texture **>(lua_touserdata(L, -1));
		lua_pushnumber(L, texture->get_buffer());
		return 1;
	}
    lua_pushnil(L);
    return 1;
...
/////////////
bool Texture::is_generated()const
{
#ifdef DOKUN_OPENGL
#ifdef __windows__
	if(!wglGetCurrentContext()) // no context (to make function more safer to use and to prevent crash)
