...
class File { // file system
	...
	// directory
	static bool                     checkdir(const std::string& path);                      // returns true if a directory exists, else false
	static std::vector<std::string> getdir  (const std::string& path, std::string filter = "*."); // returns a list of all filenames within a directory
	static bool makedir                     (const std::string& path);                      // creates a new directory and returns true if it was a success
	static std::string getcurrentdir        ();                                             // returns the current directory
	// properties
	private:
	    // file content
		std::string file;
